...
运行资源的来源与目录要求见 [第三方资源](THIRD_PARTY.md)。输出位于 `release/`；Release 只上传完整 `.desktop` 安装文件及 `.desktop.sha256`。`.sh` 与 ZIP 是本地打包中间产物，无需上传。GitHub 自动生成的 Source code ZIP 不能直接安装。
...
仓库根目录的 `install.sh` 是在线下载入口，默认获取 Latest，自动下载 `.desktop` 和 `.desktop.sha256`，校验后通过文件内的启动器安装。在线安装与双击安装共用同一份资源；可通过 `--version v0.1.0-alpha.9` 固定版本。双击安装只需下载 `.desktop` 文件。
