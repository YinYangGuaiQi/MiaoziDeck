...
公开仓库：[YinYangGuaiQi/MiaoziDeck](https://github.com/YinYangGuaiQi/MiaoziDeck)。About 简介：
...
当前作为普通 Release 发布并设为 Latest，便于下载。版本仍为 alpha 测试版，尚未完成全新 Steam Deck 真机验证；Latest 不代表稳定性认证。
...
README 的下载入口指向 Latest，在线安装也默认跟随 Latest，无需用户登录 GitHub。Release 显示名称可以修改，不影响安装；标签与对应安装文件的版本号应保持一致。需要安装指定版本时，向在线入口传入 `--version v0.1.0-alpha.9`。
